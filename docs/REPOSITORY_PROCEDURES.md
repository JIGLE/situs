...
Applies equally to humans and to AI coding sessions.
...
| `claude/<id>`    | Assigned by the session tool | `claude/open-prs-x9ymhy`   |
...
**One change per PR.** Branch from `origin/main`, or take the `claude/<id>` branch the session tool
assigns; do one change, open one PR, let it merge. Then delete the branch — or, for a `claude/<id>`
branch the session keeps using, restart it from `origin/main` (§2) before the next change. A branch
that outlives its PR accumulates unrelated work nobody can review as a unit.
...
`main` that changes more than docs and pushes `ghcr.io/jigle/situs:main` and `:sha-<short>`. Those
tags never claim a release.

**Only a release writes `:<version>` and `:latest`:**

1. Dispatch `release.yml` — it bumps versions on `release/vX.Y.Z` and opens a PR.
2. Merge that PR — `publish` tags the commit `vX.Y.Z`.
3. The tag starts `deploy-ghcr.yml`, which publishes `:<version>` and `:latest`.

Step 3 happens on its own only when the `RELEASE_TOKEN` secret is set: GitHub starts no workflow
from a tag pushed with the default `GITHUB_TOKEN`. Without it, dispatch `deploy-ghcr.yml` by hand
