/**
 * The countries a tax number or an identity document can come from, for AT's `pais` field: ISO
 * 3166-1 alpha-2 codes, which is what AT's webservice takes.
 *
 * Names come from `Intl.DisplayNames` in the app's locale (never the browser's), so all four
 * languages have them without catalogue entries. Intl cannot enumerate regions, hence the list.
 */
...
/** Every country, named in `locale` and sorted by name, with Portugal first. */
export function countryOptions(locale: string): CountryOption[] {
  ...
  const options = COUNTRY_CODES.map((code) => ({ code, name: names?.of(code) ?? code })).sort(
    (a, b) => collator.compare(a.name, b.name),
  );
  // Nearly every NIF here is Portuguese.
